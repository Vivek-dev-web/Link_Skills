...
import { Inbox, User, Calendar, Video, MessageSquare, FileText, Clock, Briefcase, GraduationCap } from "lucide-react";
...
function Badge({ label, color }: { label: string; color: "teal" | "amber" | "coral" | "muted" }) {
  return (
    <span className={cn(
      "inline-block text-[10px] font-semibold rounded-full px-2.5 py-0.5 uppercase tracking-wide",
      color === "teal"  && "bg-teal/10 text-teal",
      color === "amber" && "bg-amber/10 text-amber-dark",
      color === "coral" && "bg-coral/10 text-coral",
      color === "muted" && "bg-paper border border-border text-muted",
    )}>
      {label}
    </span>
  );
    ...
    <div className="card p-5 space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <Badge
            label={isStudent ? "Student" : "Professional"}
            color={isStudent ? "teal" : "amber"}
          />
      ...
      <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2 text-xs text-muted">
        {isStudent ? (
          <>
            {c.college && (
              <div><span className="font-medium text-ink">College: </span>{c.college}</div>
            )}
            {c.yearOfStudy && (
              <div><span className="font-medium text-ink">Year: </span>{c.yearOfStudy}</div>
            )}
            {c.interestArea && (
              <div><span className="font-medium text-ink">Interest: </span>{c.interestArea}</div>
            )}
            {c.timeSlot && (
              <div className="flex items-center gap-1">
                <Calendar size={10} />
                <span className="font-medium text-ink">Slot: </span>{c.timeSlot}
              </div>
            )}
          </>
        ) : (
          <>
            {c.currentRole && (
              <div className="flex items-center gap-1">
                <Briefcase size={10} />
                <span className="font-medium text-ink">Role: </span>{c.currentRole}
              </div>
            )}
            {c.yearsExp != null && (
              <div><span className="font-medium text-ink">Experience: </span>{c.yearsExp} yrs</div>
            )}
            {c.goal && (
              <div><span className="font-medium text-ink">Goal: </span>{GOAL_LABELS[c.goal] ?? c.goal}</div>
            )}
            {c.mode && (
              <div className="flex items-center gap-1">
                <ModeIcon size={10} />
                <span className="font-medium text-ink">Mode: </span>{c.mode}
              </div>
            )}
            {c.availableFrom && (
              <div className="flex items-center gap-1">
                ...
                {format(new Date(c.availableFrom), "dd MMM yyyy")}
              </div>
            )}
          </>
        )}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-teal/10 flex items-center justify-center">
            <Inbox size={20} className="text-teal" />
          </div>
          <div>
            <h1 className="font-display text-2xl text-ink">Consultation Requests</h1>
            <p className="text-sm text-muted">{list.length} total request{list.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
      </div>
        ...
        {(["ALL", "STUDENT", "PROFESSIONAL"] as const).map((t) => (
          <button
            ...
            className={cn(
              ...
              filter === t
                ? "bg-teal text-white"
                : "bg-paper border border-border text-muted hover:border-teal/40"
            )}
          >
